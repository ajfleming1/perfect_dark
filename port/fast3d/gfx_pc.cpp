...
static size_t tex_upload_buffer_size = 0;
...
static float buf_vbo[MAX_BUFFERED * (64 * 3)]; // 3 vertices in a triangle and 64 floats per vtx
...
static bool gfx_texture_cache_lookup(int i, const TextureCacheKey& key) {
    ...
    if (gfx_texture_cache.map.size() >= TEXTURE_CACHE_MAX_SIZE) {
        ...
        const TextureCacheKey& old_key = gfx_texture_cache.lru.front();
        it = gfx_texture_cache.map.find(old_key);
        if (it != gfx_texture_cache.map.end()) {
    ...
    node->second.lru_location = gfx_texture_cache.lru.insert(gfx_texture_cache.lru.end(), key);
...
static void import_texture_rgba16(int tile, const LoadedTexture& loaded_texture, bool importReplacement) {
    ...
    if (size_bytes * 2 > tex_upload_buffer_size) {
        sysLogPrintf(LOG_ERROR, "F3D: Texture too large for upload buffer! %u > %u", size_bytes * 2, tex_upload_buffer_size);
        return;
    }
...
static void import_texture_rgba32(int tile, const LoadedTexture& loaded_texture, bool importReplacement) {
    ...
    if (size_bytes > tex_upload_buffer_size) {
        sysLogPrintf(LOG_ERROR, "F3D: Texture too large for upload buffer! %u > %u", size_bytes, tex_upload_buffer_size);
        return;
    }
    ...
    const uint32_t width = rdp.texture_tile[tile].line_size_bytes / 4;
    const uint32_t height = size_bytes / rdp.texture_tile[tile].line_size_bytes;
    gfx_rapi->upload_texture(tex_upload_buffer, width, height);
...
static void import_texture_ia4(int tile, const LoadedTexture& loaded_texture, bool importReplacement) {
    ...
    if (size_bytes * 8 > tex_upload_buffer_size) {
        sysLogPrintf(LOG_ERROR, "F3D: Texture too large for upload buffer! %u > %u", size_bytes * 8, tex_upload_buffer_size);
        return;
    }

    uint8_t *dest = tex_upload_buffer;
    for (uint32_t i = 0, j = 0; j < size_bytes; j += full_image_line_size_bytes - line_size_bytes) {
        for (uint32_t k = 0; k < line_size_bytes; k++, j++) {
            const uint8_t byte = addr[j];
            
            uint8_t part = (byte >> 4) & 0xf;
            uint8_t intensity = part >> 1;
            uint8_t alpha = part & 1;
            uint8_t c = SCALE_3_8(intensity);
            dest[0] = c; dest[1] = c; dest[2] = c; dest[3] = alpha ? 255 : 0;
            dest += 4;

            part = byte & 0xf;
            intensity = part >> 1;
            alpha = part & 1;
            c = SCALE_3_8(intensity);
            dest[0] = c; dest[1] = c; dest[2] = c; dest[3] = alpha ? 255 : 0;
            dest += 4;
        }
    }
...
static void import_texture_ia8(int tile, const LoadedTexture& loaded_texture, bool importReplacement) {
    ...
    if (size_bytes * 4 > tex_upload_buffer_size) {
        sysLogPrintf(LOG_ERROR, "F3D: Texture too large for upload buffer! %u > %u", size_bytes * 4, tex_upload_buffer_size);
        return;
    }
    ...
    for (uint32_t i = 0, j = 0; j < size_bytes; j += full_image_line_size_bytes - line_size_bytes) {
        for (uint32_t k = 0; k < line_size_bytes; k++, j++) {
            const uint8_t val = addr[j];
            const uint8_t intensity = SCALE_4_8(val >> 4);
            const uint8_t alpha = SCALE_4_8(val & 0xf);
            dest[0] = intensity; dest[1] = intensity; dest[2] = intensity; dest[3] = alpha;
            dest += 4;
            i++;
        }
    }
...
static void import_texture_i4(int tile, const LoadedTexture& loaded_texture, bool importReplacement) {
    ...
    for (uint32_t i = 0, j = 0; j < size_bytes; j += full_image_line_size_bytes - line_size_bytes) {
        for (uint32_t k = 0; k < line_size_bytes; k++, j++) {
            ...
            uint8_t part = (byte >> 4) & 0xf;
            uint8_t intensity = SCALE_4_8(part);
            dest[0] = intensity; dest[1] = intensity; dest[2] = intensity; dest[3] = intensity;
            dest += 4;

            part = byte & 0xf;
            intensity = SCALE_4_8(part);
            dest[0] = intensity; dest[1] = intensity; dest[2] = intensity; dest[3] = intensity;
            dest += 4;
        }
    }
...
static void import_texture_i8(int tile, const LoadedTexture& loaded_texture, bool importReplacement) {
    ...
    for (uint32_t i = 0, j = 0; j < size_bytes; j += full_image_line_size_bytes - line_size_bytes) {
        for (uint32_t k = 0; k < line_size_bytes; k++, j++) {
            const uint8_t intensity = addr[j];
            dest[0] = intensity; dest[1] = intensity; dest[2] = intensity; dest[3] = intensity;
            dest += 4;
            i++;
        }
...
static void import_texture_ci4(int tile, const LoadedTexture& loaded_texture, bool importReplacement) {
    ...
    for (uint32_t i = 0, j = 0; j < size_bytes; j += full_image_line_size_bytes - line_size_bytes) {
        for (uint32_t k = 0; k < line_size_bytes; k++, j++) {
            const uint8_t byte = addr[j];
            const uint8_t idx1 = (byte >> 4) & 0xf;
            const uint8_t idx2 = byte & 0xf;
            palette_to_rgba32(palette[idx1], tex_upload_buffer + 4 * i++);
            palette_to_rgba32(palette[idx2], tex_upload_buffer + 4 * i++);
        }
    }
...
static void gfx_sp_vertex(size_t n_vertices, size_t dest_index, const Vtx* vertices) {
    ...
    for (size_t i = 0; i < n_vertices; i++, dest_index++) {
        ...
        static const struct NormalColor default_color = { 0, 0, 0, 255 };
        const struct NormalColor *vcn = &default_color;
        if (rsp.vertex_colors) {
            uint32_t vcn_idx = v->colour >> 2;
            if (vcn_idx < MAX_VERTEX_COLORS) {
                vcn = &rsp.vertex_colors[vcn_idx];
            }
        }
...
static void gfx_sp_tri1(uint32_t vtx1_idx, uint32_t vtx2_idx, uint32_t vtx3_idx, bool is_rect) {
    ...
    if (prg == NULL) {
        return; // shader generation failed; skip this triangle
    }
    ...
    if (buf_vbo_len + 64 * 3 > MAX_BUFFERED * 64 * 3) {
        gfx_flush();
    }

    for (int i = 0; i < 3; i++) {
        float z = v_arr[i]->z, w = v_arr[i]->w;
        if ((rdp.other_mode_l & G_ZS_PRIM) == G_ZS_PRIM) {
            if (clip_parameters.z_is_from_0_to_1) {
                z = rdp.prim_depth * w;
            } else {
                z = (rdp.prim_depth * 2.0f - 1.0f) * w;
            }
...
static void gfx_dp_set_texture_image(uint32_t format, uint32_t size, uint32_t width, uint32_t tex_flags, const void* addr) {
    static const uint8_t dummy_addr[4] = { 0 };
    rdp.texture_to_load.addr = addr ? (const uint8_t*)addr : dummy_addr;
...
static void gfx_dp_load_block(uint8_t tile, uint32_t uls, uint32_t ult, uint32_t lrs, uint32_t dxt) {
    ...
    if (dxt > 0) {
        loaded_texture.full_image_line_size_bytes = (2048 * 8) / dxt;
    } else {
    ...
    for (;;) {
        ...
        switch (opcode) {
            ...
            case G_SETENVCOLOR:
                ...
                break;
            case G_SETPRIMDEPTH:
                rdp.prim_depth = ((cmd->words.w1 >> 16) & 0xFFFF) / 65535.0f;
                break;
            case G_SETPRIMCOLOR:
...
extern "C" void gfx_init(const GfxInitSettings *settings) {
    ...
    if (tex_upload_buffer == nullptr) {
        // We cap texture max to 2k, because why would you need more?
        int max_tex_size = std::min(2048, gfx_rapi->get_max_texture_size());
        tex_upload_buffer_size = max_tex_size * max_tex_size * 4;
        tex_upload_buffer = (uint8_t*)malloc(tex_upload_buffer_size);
        if (!tex_upload_buffer) {
            sysLogPrintf(LOG_ERROR, "F3D: Failed to allocate texture upload buffer!");
            return;
        }
    }
