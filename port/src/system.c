...
{
#ifdef PLATFORM_WIIU
	return OSTicksToMicroseconds(OSGetTime()) - startTick;
#else
	struct timeval tv;
...
#ifdef PLATFORM_WIIU
	WHBLogPrintf("%s%s", prefix[level], logmsg);
#endif
...
{
#ifdef PLATFORM_WIIU
	// On Wii U, use a fixed path for SD card homebrew
	// WUHB/RPX files are typically run from fs:/vol/external01 (SD card)
	const char *wiiuPath = "fs:/vol/external01/wiiu/apps/perfectdark";
	strncpy(outPath, wiiuPath, outLen - 1);
	outPath[outLen - 1] = '\0';
	return;
#else
	// try asking SDL
...
{
#ifdef PLATFORM_WIIU
	// On Wii U, use the same path as executable for saves
	const char *wiiuPath = "fs:/vol/external01/wiiu/apps/perfectdark";
	strncpy(outPath, wiiuPath, outLen - 1);
	outPath[outLen - 1] = '\0';
	return;
...
void sysSleep(const s64 hns)
{
	...
	// hns is in 100ns units; convert to microseconds for WUT
	OSSleepTicks(OSMicrosecondsToTicks((u64)hns / 10));
#else
