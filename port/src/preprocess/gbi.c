...
void gbiGdlRewriteAddrs(u8 *dst, u32 offset)
{
	...
	do {
		...
		// On 32-bit BE (no-swap layout): cmd = w0<<32|w1, opcode is in bits [63:56].
		// On 32-bit LE (swap layout):    cmd = w1<<32|w0, opcode is in bits [31:24].
		// On 64-bit (two-word layout):   *cmds = w0 only (32 bits), opcode in bits [31:24].
#if HOST_DWORDS_PER_CMD == 2
		idx = 1;
		u8 opcode = (cmd >> 24) & 0xff;
#elif defined(PLATFORM_BIG_ENDIAN)
		u8 opcode = (cmd >> 56) & 0xff;
#else
		u8 opcode = (cmd >> 24) & 0xff;
#endif
		cmd = (u64)opcode << 56;
...
#else
		// On 32-bit little-endian: swap w0/w1 so Gfx struct reads correctly.
		// On 32-bit big-endian (Wii U/PPC): N64 is also big-endian, so w0 is
		// already the high word and w1 the low word — store as-is.
