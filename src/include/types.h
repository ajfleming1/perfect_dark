...
struct packedpad {
	...
	// On big-endian (PPC/N64), bitfields are allocated from MSB.
	// The N64 ROM layout is flags(18)|room(10)|liftnum(4) from MSB to LSB,
	// so the declaration order must match that.
	s32 flags : 18;
	s32 room : 10;
	s32 liftnum : 4;
#else
