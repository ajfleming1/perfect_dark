/*  gx2_shader_gen.c - Fast3D GX2 shader generator
...
static GX2SamplerVar samplerVars[] = {
    ...
    { "uTexMask0", GX2_SAMPLER_VAR_TYPE_SAMPLER_2D, SHADER_FIRST_TEXTURE + 2 },
    { "uTexMask1", GX2_SAMPLER_VAR_TYPE_SAMPLER_2D, SHADER_FIRST_TEXTURE + 3 },
    { "uTexBlend0", GX2_SAMPLER_VAR_TYPE_SAMPLER_2D, SHADER_FIRST_TEXTURE + 4 },
    { "uTexBlend1", GX2_SAMPLER_VAR_TYPE_SAMPLER_2D, SHADER_FIRST_TEXTURE + 5 },
};
...
static int generatePixelShader(GX2PixelShader *psh, struct CCFeatures *cc_features) {
    static const size_t max_program_buf_size = 2048 * sizeof(uint64_t);
...
static int generateVertexShader(GX2VertexShader *vsh, struct CCFeatures *cc_features) {
    static const size_t max_program_buf_size = 1024 * sizeof(uint64_t);
    uint64_t *program_buf = memalign(GX2_SHADER_PROGRAM_ALIGNMENT, max_program_buf_size);
    if (!program_buf) {
        return -1;
    }
...
int gx2GenerateShaderGroup(struct ShaderGroup *group, struct CCFeatures *cc_features) {
    ...
    for (uint32_t i = 0; i < num_vs_inputs; i++) {
        bool used = false;
        GX2AttribFormat format = GX2_ATTRIB_FORMAT_FLOAT_32_32;
        uint32_t sel = GX2_COMP_SEL(_x, _y, _0, _0);
        uint32_t size = 0;

        if (i == 0) { // aVtxPos
            used = true;
            format = GX2_ATTRIB_FORMAT_FLOAT_32_32_32_32;
            sel = GX2_COMP_SEL(_x, _y, _z, _w);
            size = 4 * sizeof(float);
        } else if (i == 1 || i == 2) { // TexCoord
            int tex = i - 1;
            if (cc_features->used_textures[tex]) {
                used = true;
                bool s = cc_features->clamp[tex][0];
                bool t = cc_features->clamp[tex][1];
                if (s && t) {
                    format = GX2_ATTRIB_FORMAT_FLOAT_32_32_32_32;
                    sel = GX2_COMP_SEL(_x, _y, _z, _w);
                    size = 4 * sizeof(float);
                    ...
                    sel = GX2_COMP_SEL(_x, _y, _z, _0);
                    size = 3 * sizeof(float);
                } else if (t) {
                    format = GX2_ATTRIB_FORMAT_FLOAT_32_32_32;
                    sel = GX2_COMP_SEL(_x, _y, _0, _z);
                    size = 3 * sizeof(float);
                } else {
                    format = GX2_ATTRIB_FORMAT_FLOAT_32_32;
                    sel = GX2_COMP_SEL(_x, _y, _0, _0);
                    size = 2 * sizeof(float);
                }
            }
        } else if (i == 3) { // aFog
            ...
            int input = i - 5;
            if (input < cc_features->num_inputs) {
                used = true;
                format = cc_features->opt_alpha ? GX2_ATTRIB_FORMAT_FLOAT_32_32_32_32 : GX2_ATTRIB_FORMAT_FLOAT_32_32_32;
                sel = cc_features->opt_alpha ? GX2_COMP_SEL(_x, _y, _z, _w) : GX2_COMP_SEL(_x, _y, _z, _1);
                size = (cc_features->opt_alpha ? 4 : 3) * sizeof(float);
            }
        }

        if (used) {
            group->attributes[i] = (GX2AttribStream) { i, 0, attribOffset, format, GX2_ATTRIB_INDEX_PER_VERTEX, 0, sel, GX2_ENDIAN_SWAP_DEFAULT };
            attribOffset += size;
        } else {
            // Unused but must be present to keep register indices consistent.
            // We use a safe dummy entry that points to the start of the buffer with no-op selection.
            group->attributes[i] = (GX2AttribStream) { i, 0, 0, GX2_ATTRIB_FORMAT_FLOAT_32_32_32_32, GX2_ATTRIB_INDEX_PER_VERTEX, 0, GX2_COMP_SEL(_0, _0, _0, _0), GX2_ENDIAN_SWAP_DEFAULT };
        }
    }

    group->numAttributes = num_vs_inputs;
