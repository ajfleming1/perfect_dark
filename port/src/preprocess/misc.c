...
u8 *preprocessTexturesList(u8* data, u32 size, u32* outSize)
{
	...
	// On little-endian hosts the bitfields in struct texture are laid out
	// opposite to the N64 (big-endian) ROM format, so we need to manually
	// extract and repack each entry.  On big-endian (e.g. Wii U / PPC) the
	// compiler reads the bitfields directly in the correct N64 byte order, so
