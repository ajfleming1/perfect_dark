...
static s32 vidFramerateLimit = DEFAULT_VID_FRAMERATE;
...
s32 videoInit(void)
{
	sysLogPrintf(LOG_NOTE, "videoInit: starting");

#ifdef PLATFORM_WIIU
	sysLogPrintf(LOG_NOTE, "videoInit: using Wii U GX2 backend");
	wmAPI = &gfx_wiiu;
	...
	sysLogPrintf(LOG_NOTE, "videoInit: setting up viewport");
	...
	sysLogPrintf(LOG_NOTE, "videoInit: calling gfx_init");
	gfx_init(&set);
	sysLogPrintf(LOG_NOTE, "videoInit: gfx_init returned");
	...
	sysLogPrintf(LOG_NOTE, "videoInit: complete");
	initDone = true;
	return 0;
}
