...
void texLoad(texnum_t *updateword, struct texpool *pool, bool unusedarg)
{
	...
	if ((*updateword & 0xffff0000) == 0 || (*updateword & 0xffff0000) == 0xabcd0000) {
		...
		if (tex == NULL) {
...
#ifndef PLATFORM_N64
			// Calculate required buffer size and allocate from heap
			texdatasize = ((uintptr_t) (nextoffset - thisoffset) + 0x1f) >> 4 << 4;
			heapcompbuffer = sysMemAlloc(texdatasize + 0x40);
			if (!heapcompbuffer) {
				sysLogPrintf(LOG_ERROR, "texLoad: failed to alloc %u bytes for texture %d", texdatasize + 0x40, g_TexNumToLoad);
				return;
			}
			alignedcompbuffer = (u8 *) (((uintptr_t)heapcompbuffer + 0xf) >> 4 << 4);
#endif
			...
			if (modTextureLoad(g_TexNumToLoad, alignedcompbuffer, texdatasize) > 0) {
